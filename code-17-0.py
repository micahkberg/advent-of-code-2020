...
#list of tuples that represent vectors to each neighbor from a given cube segment
...
for y in range(len(flat_shape)):
    for x in range(len(flat_shape[y])):
        if flat_shape[y][x] =="#":
            initial_active_cubes.append((x,y,0))

    
def get_neighbors(cube):
    neighbor_lis = []
    for neighbor in neighbors:    
        neighbor_lis.append((cube[0]+neighbor[0],cube[1]+neighbor[1],cube[2]+neighbor[2]))
